...
#### REQ-047 — "Updated by" names the assignee, not the editor
**Type:** Defect. **Backend and frontend, small.**

**Problem:** The "Updated by" field on the assessments page shows the person the assessment was **assigned to**, not the person who last **changed** it. Those differ as soon as anyone other than the assignee edits a rating, which is the normal case — so the field is wrong most of the time it matters, and **wrong in a way that reads as authoritative.** A name under "Updated by" is taken as a statement about who did the work.

**The premise was checked against the code before this was written, and the framing changes as a result.** Two things are established, and one is not:

> **✅ `updated_by` IS populated on the write path.** Both `PUT /api/assessments/{assessment_id}` (`main.py:3473`) and the bulk update (`main.py:3551`) set `a.updated_by = %s` alongside `last_updated = NOW()`. **So the "nothing else to show" branch does not apply, and populating the column is not the work.**
>
> **✅ The frontend is already asking for the right field.** `AssessmentDetail.tsx:1756` and `:1920` both read **`updated_by_name` first**, falling back through `submitted_by_name` to `assigned_to_name`. The display code is not choosing the wrong value — **its fallback chain is landing on the last rung.**
>
> **❌ `updated_by_name` does not exist anywhere in the API.** `assigned_to` and `submitted_by` each have a `LEFT JOIN` producing `assigned_to_name` and `submitted_by_name`; **`updated_by` has no such join.** `GET /api/assessments/{assessment_id}` selects the raw `a.updated_by` id (`main.py:3246`) and nothing else returns the column at all — not the grouped list, not by-aspect.

**So this is a missing join, not a mis-wired display.** The real statement of the defect: **the backend never supplies a name for `updated_by`, so a correct fallback chain resolves to the assignee.**

**Scope — backend:** add the `updated_by_name` join on the endpoints that feed these two surfaces, matching the existing `assigned_to_name` / `submitted_by_name` pattern. **Contract entry required** — this adds a field.

**Scope — frontend:** verification, most likely nothing more. The types already declare `updated_by_name` (`types/assessment.ts:104`, `:153`) and both display sites already prefer it. **Confirm rather than assume** — if a surface reads `assigned_to_name` directly, fix that one.

**Decide and state the empty case — do not let it fall out of the implementation.** `updated_by` is **only written on UPDATE, never on INSERT** (data model §15), so **an assessment nobody has edited since creation has `updated_by = NULL` legitimately.** The data model records 1142 of 1198 rows NULL as of April 2026. **The choice is between falling back to the assignee, falling back to `submitted_by`, and showing an em dash**, and it is a product decision: an em dash says "nobody has edited this", the assignee says something that is not true. **The current fallback chain answers this by accident; the requirement is to answer it on purpose.**

**Distinct from REQ-035 (M2)**, which adds an updater name to **standards** and **needs a schema change** — `mat_standards` has `created_by_user_id` and no `updated_by` at all. **This one is an existing column on an existing table**, already written to, already carrying an FK to `users`. Same words, different work.
