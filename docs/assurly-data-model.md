...
**Intervention flag** (`intervention_required` in the same endpoint) counts standards with `rating <= 2` — the bottom two rungs are "needs action" under both polarities (Inadequate / Needs work for assurance; Critical / Major risk for risk).
